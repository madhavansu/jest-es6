...
export function insertAtPosition(element, list, position) {
  // write code here
  let list2 = [...list];
  if(position < list2.length && list2[position-1] !== element) 
    list2.splice(position, 0, element)
...
export function getIndexedObject(items) {
  ...
  return items.reduce((obj, item) => ({
    ...obj, 
    [item.name]: item.props
  }) ,{});
  // return items.reduce((obj, item) => (obj[item.name] = item.props, obj) ,{});
}
...
export function getSortedDescNumeric(items) {
  // write code here
  let ar = items.reduce((obj, item) => {
    return [...new Set([].concat(...[...obj, item.values]))] 
  }, []);
  return ar.sort((a, b) => a - b).reverse();
}
...
export function sumOfValuesMoreThanN(items, n) {
  // write code here
  return items.reduce((ar, item) => {
    return ((item.values.reduce((a,b) => a + b, 0)) > n) ? [...ar, item.id] : ar;
  }, []);
}
...
export async function promiseByStrategy(a, b, strategy) {
  ...
  switch(strategy) {
    case 'a-then-b':
      ...
      break;
    case 'b-then-a':
      return Promise.all([b, a]).then(function(values) {
        return values.join("");
      });
      break;
    case 'parallel':
      ...
      break;
    case 'first':
      return Promise.race([a, b]).then(function(values) {
        return values;
      });
      break;
    default:
